...
An interesting thing here is the action we are returning - the `:caps` action. That means that we want to transmit the information about the supported caps through the `output` pad, to the next element in the pipeline. In the [chapter 3.1](3.1_Caps.md) you will find out more about caps and formats and learn why it is required to do so.
