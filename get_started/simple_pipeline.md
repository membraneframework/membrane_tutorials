...
>
> To play and work with membrane you'll need to have Elixir environment installed on your system. Instructions how to do it depending to your operating system you'll find [here](https://elixir-lang.org/install.html).
...
```
Next we can fetch them by running
```bash
$ mix deps.get
```

**lib/hello.ex** - Here we'll be creating our pipeline.
...
Let's implement a first one, `handle_init/1`. As you can see in [the documentation](https://hexdocs.pm/membrane_core/Membrane.Pipeline.html#c:handle_init/1) `handle_init` takes one argument wich can be any type - it's a way to pass some arguments needed for pipeline to start. As our app one and only purpose id to play an mp3 file we can assume that the only value we need to pass into pipeline is a path for a file we want to play:
```elixir
defmodule Hello do
...
The elements we'd like to use to play our mp3 will be:
 ...
 - `Membrane.FFmpeg.SWResample.Converter` - a converter based on [FFmdeg SWResample](https://ffmpeg.org/doxygen/trunk/group__lswr.html#details). We'll be needing it to resample our row audio stream from 24 to 16 bits.
